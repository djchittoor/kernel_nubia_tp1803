...
static void tmd2725_schedule_prox_work(struct tmd2725_chip *chip,
...
static void tmd2725_do_prox_offset_cal(struct tmd2725_chip *chip,
				       enum tmd2725_prox_state prox_state)
{
...
static void tmd2725_get_prox(struct tmd2725_chip *chip)
{
	...
	if (chip->pdata->debug_level) {
		SENSOR_LOG_INFO
		    ("prox_thres_far:%d prox_thres_near:%d prox_thres_oil_far:%d prox_thres_oil_near:%d\n",
		     chip->params.prox_thres_far, chip->params.prox_thres_near,
		     chip->params.prox_thres_oil_far,
		     chip->params.prox_thres_oil_near);
		SENSOR_LOG_INFO
		    ("th_min:%d th_max:%d raw:%d state:%d detected:%s\n",
		     chip->params.prox_th_min, chip->params.prox_th_max,
		     chip->prx_inf.raw, chip->prox_state,
	...
	if (chip->prx_inf.detected == PROX_FAR) {
		...
		if (chip->prx_inf.raw < chip->params.prox_th_min) {
			if (chip->prox_state == PROX_STATE_NONE) {
				...
				if (chip->params.prox_oil_state) {
					SENSOR_LOG_DEBUG_IF(chip->pdata->
							    debug_level,
							    "!!OIL FAR-->FAR\n");
					chip->params.prox_oil_state = OIL_NONE;
					tmd2725_schedule_prox_work(chip,
								   PROX_STATE_CALIB);
				}
				SENSOR_LOG_INFO
				    ("prx_inf.raw :%d detected:%d \n",
		...
		if (chip->prx_inf.raw < chip->params.prox_th_min) {
			...
			SENSOR_LOG_INFO
			    ("chip->prx_inf.raw :%d prox_thres_far:%d",
			...
			if (chip->prox_state == PROX_STATE_NONE) {
				...
				if (chip->params.prox_th_max != PS_DATA_MAX) {
					...
					SENSOR_LOG_DEBUG_IF(chip->pdata->
							    ...
							    "!!OIL NEAR-->FAR\n");
					chip->params.prox_th_min =
					    chip->params.prox_thres_near - 10;
					chip->params.prox_th_max =
					    chip->params.prox_thres_oil_far +
					    10;
					SENSOR_LOG_INFO
...
static void tmd2725_report_prox(struct tmd2725_chip *chip)
{
	if (chip->p_idev) {
		SENSOR_LOG_INFO("detect:%d,last:%d\n", chip->prx_inf.detected,
				chip->prx_inf.last_detected);
		if (chip->prx_inf.detected != chip->prx_inf.last_detected) {
			input_report_rel(chip->p_idev, REL_RZ,
					 chip->prx_inf.detected);
...
static int resume_error_restore(struct tmd2725_chip *chip)
{
	...
	if (chip->resume_error) {
		res =
		    ams_i2c_read(chip->client, TMD2725_REG_STATUS,
				 &chip->shadow[TMD2725_REG_STATUS]);
		if (res <= 0) {
			SENSOR_LOG_ERROR
			    ("i2c read failed while restore resume error\n");
			return -1;
		}
		status = chip->shadow[TMD2725_REG_STATUS];
		if (status != 0)
			ams_i2c_write(chip->client, sh, TMD2725_REG_STATUS,
				      status);
...
static int tmd2725_configure_prox_mode(struct tmd2725_chip *chip, u8 state)
{
	...
	if (state)		// Turning on prox
	{
		...
		ams_i2c_write(client, sh, TMD2725_REG_PGCFG1,
			      chip->params.prox_gain | chip->params.prox_drive);
		...
		ams_i2c_write(client, sh, TMD2725_REG_INTENAB, TMD2725_PIEN);
...
static int tmd2725_prox_thres_calibrate(struct tmd2725_chip *chip)
{
	...
	if (prox_thres_near < PS_THRESH_DATA_MIN
	    || prox_thres_near > PS_THRESH_DATA_MAX) {
...
static ssize_t tmd2725_device_prox_raw(struct device *dev,
...
static ssize_t tmd2725_device_prox_detected(struct device *dev,
				       ...
				       const char *buf, size_t size)
{
	...
	chip->params.prox_gain =
	    chip->shadow[TMD2725_REG_PGCFG1] & TMD2725_MASK_PGAIN;
					  ...
					  const char *buf, size_t size)
{
	...
	AMS_MUTEX_LOCK(&chip->ps_lock);
	ams_i2c_modify(chip, chip->shadow, TMD2725_REG_PERS,
		       TMD2725_MASK_PROX_PERS, persist);
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE,
			...
			rPdata,
			chip->prx_inf.detected ==
			PROX_NEAR ? "Prox Detect" : "Prox Release",
			chip->amsIndoorMode ? "Indoor" : "Outdoor");
}

static ssize_t tmd2725_prox_regs_store(struct device *dev,
				       struct device_attribute *attr,
...
static struct device_attribute tmd2725_prox_attrs[] = {
	__ATTR(enable, 0644, tmd2725_prox_enable_show,
	       tmd2725_prox_enable_store),
	...
	__ATTR(prox_debug, 0644, tmd2725_prox_raw_debug_show,
	       tmd2725_prox_raw_debug_store),
	__ATTR(prox_offset_cal, 0644, tmd2725_prox_uncover_cal_show,
	       tmd2725_prox_uncover_cal_store),
	__ATTR(prox_thres, 0644, tmd2725_prox_thres_show,
	       tmd2725_prox_thres_store),
	__ATTR(prox_thres_max, 0440, tmd2725_prox_max_thres_show, NULL),
	__ATTR(prox_thres_min, 0440, tmd2725_prox_min_thres_show, NULL),
	__ATTR(prox_init, 0644, tmd2725_prox_dev_init_show,
	       tmd2725_prox_dev_init_store),
	__ATTR(prox_data_max, 0440, tmd2725_prox_max_data_show, NULL),
	__ATTR(prox_uncover_min, 0440, tmd2725_prox_uncover_data_min_show,
	       NULL),
	__ATTR(prox_uncover_max, 0440, tmd2725_prox_uncover_data_max_show,
	       NULL),
	__ATTR(prox_raw, 0440, tmd2725_device_prox_raw, NULL),
	__ATTR(prox_detect, 0440, tmd2725_device_prox_detected, NULL),
	__ATTR(prox_gain, 0644, tmd2725_prox_gain_show,
	       tmd2725_prox_gain_store),
	__ATTR(prox_offset, 0440, tmd2725_prox_offset_show, NULL),
	__ATTR(prox_persist, 0644, tmd2725_prox_persist_show,
	       tmd2725_prox_persist_store),
	__ATTR(prox_regs, 0640, tmd2725_prox_regs_show,
	       tmd2725_prox_regs_store),
	__ATTR(prox_thres_to_persist, 0220, NULL,
	       tmd2725_prox_thres_to_persist_store),
};
...
static int tmd2725_irq_handler_locked(struct tmd2725_chip *chip, u8 status)
{
	...
	if (status & TMD2725_ST_PRX_IRQ) {
		...
		if (chip->prx_inf.prox_raw_debug) {
			input_report_rel(chip->p_idev, REL_MISC,
					 chip->prx_inf.raw);
			input_sync(chip->p_idev);
			return ret;
		}
...
static int tmd2725_prox_irq_init(struct tmd2725_chip *chip)
{
	...
	if (gpio_is_valid(chip->irq_gpio)) {
		...
		ret =
		    gpio_request_one(chip->irq_gpio, GPIOF_DIR_IN,
				     "tmd2725_irq_gpio");
		if (ret) {
			       ...
			       struct i2c_driver *driver)
{
	...
	/* create workqueue */
	chip->ps_workqueue =
	    create_singlethread_workqueue("tmd2725_irq_workqueue");
	if (IS_ERR_OR_NULL(chip->ps_workqueue)) {
		ret = -ENOMEM;
		SENSOR_LOG_ERROR("cannot create work taos_work_queue, ret = %d",
				 ret);
		goto exit_input_dev;
	}
	/* create sysfs */
	...
	chip->ps_dev =
	    device_create(ps_class, 0, tmd2725_ps_dev_t, driver, DEV_PS_NAME);
...
void tmd2725_ps_device_unregister(struct tmd2725_chip *chip)
{
	...
	sensor_remove_sysfs_interfaces(chip->ps_dev, tmd2725_prox_attrs,
				       ARRAY_SIZE(tmd2725_prox_attrs));
