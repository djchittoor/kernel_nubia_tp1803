...
static int sf_ctl_spi_speed(unsigned int speed)
{
	...
	if (speed <= 1000 * 1000) {
		...
		delay_ns =
		    (1.0 / (((double)speed) / (1.0 * 1000 * 1000)) -
		     1.0 / 4.8) * 8 * 1000;
...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SF_IOC_REPORT_KEY_EVENT:{
			if (copy_from_user
			    (&kevent, (sf_key_event_t *) arg,
			     sizeof(sf_key_event_t))) {
				xprintk(KERN_ERR,
	...
	case SF_IOC_ATTRIBUTE:{
			err =
			    __put_user(sf_ctl_dev.attribute,
				       (__u32 __user *) arg);
			break;
		}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	len +=
	    sprintf((char *)buf,
		    ...
		    sf_hw_ver.algorithm, sf_hw_ver.firmware);
	return len;
}
...
static int sf_suspend(void)
{
	...
	kobject_uevent_env(&sf_ctl_dev.miscdev.this_device->kobj, KOBJ_CHANGE,
			   screen);
#if SF_INT_TRIG_HIGH
 ...
 */
static int sf_adf_event_handler(struct notifier_block *nb, unsigned long action,
				void *data)
{
...
static int sf_probe(sf_device_t * dev)
{
	...
	err = -1;
	xprintk(KERN_INFO,
		"CONFIG_RSEE not define, skip rsee_client_get_fpid!\n");
#endif
#else
	//sf_ctl_dev.spi_clk_on(true);
	...
	err =
	    sysfs_create_group(&sf_ctl_dev.miscdev.this_device->kobj,
			       &sf_attribute_group);
	...
	if (err < 0) {
		xprintk(KERN_ERR,
			"register adf notifier fail, cannot sleep when screen off");
	} else {
...
#if SF_SPI_TRANSFER
static int tee_spi_transfer(void *sf_conf, int cfg_len, const char *txbuf,
			    char *rxbuf, int len)
{
			   ...
			   loff_t * f_pos)
{
...

	{
		...
		int cfg_len = 0;
		status =
		    tee_spi_transfer(sf_conf, cfg_len, sf_ctl_dev.spi_buffer,
...
static int sf_read_sensor_id(void)
{
	...
	do {
		...
		ret =
		    tee_spi_transfer(&smt_conf, cfg_len, writebuf, readbuf, 6);
		...
		if ((0x53 == readbuf[2]) && (0x75 == readbuf[3])
		    && (0x6e == readbuf[4])
		    && (0x57 == readbuf[5])) {
		...
		writebuf[4] = 0x00;
		ret =
		    tee_spi_transfer(&smt_conf, cfg_len, writebuf, readbuf, 7);
		...
		do {
			...
			writebuf[4] = 0x84;
			ret =
			    tee_spi_transfer(&smt_conf, cfg_len, writebuf,
					     readbuf, 5);

			if (ret != 0) {
				xprintk(KERN_ERR,
					"SPI transfer failed step 1\n");
				continue;
			}
			...
			if (ret != 0) {
				xprintk(KERN_ERR,
					"SPI transfer failed step 3\n");
				continue;
			}

			if (readbuf[8] == 0x82) {
				xprintk(KERN_INFO,
					"read chip is ok: 0x%02x%02x\n",
					readbuf[8], readbuf[9]);
				memset(readbuf, 0, sizeof(readbuf));
				memset(writebuf, 0, sizeof(writebuf));
				writebuf[0] = 0x32;
				...
				writebuf[4] = 0x83;
				ret =
				    tee_spi_transfer(&smt_conf, cfg_len,
						     writebuf, readbuf, 5);
				return 0;
			} else {
