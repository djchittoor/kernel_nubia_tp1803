...
static int sf_ctl_device_reset(void)
{
	...
	err =
	    pinctrl_select_state(sf_pinctrl,
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
	...
	sf_ctl_dev = ctl_dev;
	xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __FUNCTION__);
	ctl_dev->pdev->dev.of_node =
	    of_find_compatible_node(NULL, NULL, COMPATIBLE_SW_FP);
	...
	xprintk(KERN_INFO, "irq number is %d.\n", ctl_dev->irq_num);
	{
		...
		for (i = SF_PIN_STATE_INT_SET + 1; i < SF_PIN_STATE_MAX; ++i) {
			err =
			    pinctrl_select_state(sf_pinctrl, sf_pin_states[i]);
...
#else
	ctl_dev->rst_num =
	    of_get_named_gpio(ctl_dev->pdev->dev.of_node, COMPATIBLE_RESET_GPIO,
			      0);
	ctl_dev->irq_pin =
	    of_get_named_gpio(ctl_dev->pdev->dev.of_node, COMPATIBLE_IRQ_GPIO,
			      0);
	xprintk(KERN_INFO, "reset pin number = %d\n", ctl_dev->rst_num);
	xprintk(KERN_INFO, "irq pin number = %d\n", ctl_dev->irq_pin);
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
	ctl_dev->pwr_num =
	    of_get_named_gpio(ctl_dev->pdev->dev.of_node, COMPATIBLE_PWR_GPIO,
			      0);
